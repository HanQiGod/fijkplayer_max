...
fijkplayer_max 是 ijkplayer 的 Flutter 封装， 是一款支持 android 和 iOS 的 Flutter 媒体播放器插件，
由 [ijkplayer](https://github.com/befovy/ijkplayer) 底层驱动。  
通过纹理（Texture）接入播放器视频渲染到 Flutter 中。  

让我们开始使用 fijkplayer_max 构建你的 Flutter 音视频应用之旅！  
...
在 flutter 项目配置文件 `pubspec.yaml` 中加入 `fijkplayer_max` 依赖。最新的稳定版本号见 badge 图标。
...
### 安装 fijkplayer_max 依赖

进入上一步中创建的目录 playerapp 中，编辑其中的文件 `pubspec.yaml`，增加依赖 `fijkplayer_max: ^{latest verison}`
...
安装完 fijkplayer_max 之后我们先验证一下，确保安装正确。

编辑 `lib/main.dart` 文件，在开头加入如下行，引入 fijkplayer_max。
```dart
...
#### **注意事项**
* fijkplayer_max 中使用了 androidx 的 support 库，在 app 中需要启用 androidx
...
Flutter 中安装 fijkplayer_max 的内容到这里就结束了，有任何问题请在 [github issues](https://github.com/befovy/fijkplayer/issues) 中进行提问。    
